...
class FlowMapperTest(unittest.TestCase):
    ...
    def get_flow_op_list(self) -> List[FlowOperationEntity]:
        return [
            ...
            FlowOperationEntity(name="Flow_op_8", input="Input", process="Process", output="Output", join="join",
                                args="Args", where="*"),
            ...
            FlowOperationEntity(name="Flow_op_13", input="Input", process="Process", output="Output", join="join",
                                where="*"),
            FlowOperationEntity(name="Flow_op_14", input="Input", process="Process", output="Output", args="Args",
                                where="*"),
            FlowOperationEntity(name="Flow_op_15", input="Input", process="Process", join="join", args="Args", where="*"),
            FlowOperationEntity(name="Flow_op_16", input="Input", process="Process", output="Output", join="join",
                                args="Args", where="*"),
        ]
    ...
    def test_list_flow_operation_entity_to_json(self):
        ...
        flow_op_list = self.get_flow_op_list()
        self.assertEqual(json.loads(flow_op_list_json_str),
                         json.loads(self.flow_mapper.flow_operation_to_json(flow_op_list)))
